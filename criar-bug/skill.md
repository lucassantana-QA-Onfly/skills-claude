...
Use `Atlassian:addCommentToJiraIssue` na tarefa original com a descrição gerada no passo 3.

### 6. Marcar "Need fix" na tarefa original
Use `Atlassian:editJiraIssue` na tarefa original para marcar "Need fix"
no campo "Checkbox":
...
Use `Atlassian:addCommentToJiraIssue` na tarefa original com um comentário ADF mencionando o assignee salvo no passo 1:
...
Use `Atlassian:addCommentToJiraIssue` no **ticket Fix criado** (ex: TEST-456) com um comentário ADF mencionando o assignee salvo no passo 1:
...
Use `Atlassian:createJiraIssue` com:
- **Projeto**: CHEER
- **Tipo de issue**: Bug
